...
func (r *Relayer) SkywayHandleBatchSendEvent(ctx context.Context, locker sync.Locker) error {
...
func (r *Relayer) handleBatchSendEvents(ctx context.Context, processors []chain.Processor) error {
	...
	for _, p := range processors {
		...
		logger := liblog.WithContext(ctx).WithFields(log.Fields{
			...
			"action":             "handle-skyway-batch-send-events",
		})
		...
		logger = logger.WithFields(log.Fields{
			...
			"skyway-nonces": slice.Map(batchSendEvents, func(event chain.BatchSendEvent) uint64 {
